...
* **distanceType**: the function used to determine the distance between two vectors. Currently, only (squared) Euclidean distance (the default) is supported:  `d(v, w) = length(v - w)`
