...
Use `build()` to create a re-usable query instance. To then retrieve all results use `find()` on the query. More options on retrieving results are discussed later in [#run-a-query](queries#run-a-query "mention").
...
<div className="custom-nav-card">
  <a href="/data-observers-and-rx" className="custom-nav-card-link">
    <div className="custom-nav-card-content">
      <h3 className="custom-nav-card-title">Data Observers And Rx</h3>
      <p className="custom-nav-card-description">Learn more about data observers and rx</p>
    </div>
    <div className="custom-nav-card-arrow">›</div>
  </a>
</div>
