...
If your application requires other ID types (such as a string UID given by a server), you can model them as standard properties and use [queries](../queries) to look up entities by your application-specific ID.
...
:::info
This is not recommended. Check if using a [unique indexed property](../entity-annotations#object-ids-id) is a viable alternative.
:::
...
This will allow putting an entity with any valid ID (see [#special-object-ids](object-ids#special-object-ids "mention")). If the `@Id` field is writable, it can still be set to zero to let ObjectBox auto-assign a new ID.
...
For details see the documentation about [updating relations](../relations#updating-relations).
:::
