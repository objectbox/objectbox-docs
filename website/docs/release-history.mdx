...
* Added examples on how to use Kotlin's unsigned integer types to [Custom types](advanced/custom-types).
...
* [**Support Flex properties**](advanced/custom-types#flex-properties)**.** Expanding on the string and flexible map support in `3.0.0`, it is now possible to add a property using `Object` in Java or `Any?` in Kotlin. These "flex properties" allow to store values of various types like integers, floating point values, strings and byte arrays. Or lists and maps (using string keys) of those.
...
* [A new experimental Query API](queries#build-a-query) provides support for nested AND and OR conditions. [#201](https://github.com/objectbox/objectbox-java/issues/201)
...
* Add Kotlin infix extension functions for creating conditions using the[ new Query API](queries#build-a-query). See [the documentation](queries#build-a-query) for examples.
...
* [A new Query API](queries#build-a-query) provides support for nested AND and OR conditions. See [the documentation](queries#build-a-query) for examples and notable changes. [GH#201](https://github.com/objectbox/objectbox-java/issues/201)
...
* Android: the [ObjectBox LiveData](android/livedata-architecture-components) and [Paging integration](android/paging-architecture-components) migrated from Android Support Libraries to Jetpack (AndroidX) Libraries. If you are using them the library will not work unless you make the following changes in your app:
...
ObjectBox 2.0 introduces [index types](entity-annotations#index-types-string) for String. Before, every index used the property **value** for all look-ups. Now, ObjectBox can also use a **hash** to build an index. Because `String` properties are typically taking more space than scalar values, ObjectBox **switched the default index type to hash for strings.**
...
* Unique constraint for properties via [@Unique annotation](entity-annotations#unique-constraints)
* Support for char type (16 bit)
* [RX lib](data-observers-and-rx#objectbox-rxjava-extension-library) deployed in JCenter
...
* [Property based count](queries#aggregate-values): query for non-null or unique occurrences of entity properties (non-null and unique)&#x20;
...
* Query across relation bounds [using links](queries#add-query-conditions-for-related-entities-links) (aka "join"): queries just got much more powerful. For example, query for orders that have a customer with an address on "Sesame Street". Or all persons, who have a grand parent called "Alice".
* [Backlinks for to-many relations](relations#access-many-to-many-in-the-reverse-direction): now ObjectBox is "relation complete" with a bi-directional many-to-many relation.
* Query performance improvements: getting min/max values of indexed properties in constant time
* Android: added [Paging library support](android/paging-architecture-components) (architecture components)
* [Kotlin extensions](kotlin-support#using-the-provided-extension-functions): more Kotlin fun with ObjectBox KTX
* [Query parameters aliases](queries#reuse-queries-and-parameters): helps setting query parameters in complex scenarios (e.g. for properties of linked entities)
...
* Full support for [Android local tests](android/android-local-unit-tests): use full ObjectBox features in local tests
* New count method optimized for a given maximum count
* Gradle option to [define the package for MyObjectBox](advanced/advanced-setup#change-the-myobjectbox-package) explicitly
...
* Gradle plugin detects [plain Java projects](java-desktop-apps#objectbox-–-embedded-database-for-java-desktop-apps) and configures dependencies
...
* [ObjectBoxLiveData](android/livedata-architecture-components#objectbox-livedata-with-android-architecture-components): Implements LiveData from Android Architecture Components
